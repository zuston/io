 ...
 private:
  void LoadAndBind(TF_Status* status) {
    auto TryLoadAndBind = [this](const char* name, void** handle,
                                 TF_Status* status) {
      *handle = LoadSharedLibrary(name, status);
...
typedef struct HDFSRandomAccessFile {
  std::string path;
  ...
  HDFSRandomAccessFile(std::string path, std::string hdfs_path, hdfsFS fs,
                       LibHDFS* libhdfs, hdfsFile handle)
      : path(std::move(path)),
...
void Cleanup(TF_RandomAccessFile* file) {
  auto hdfs_file = static_cast<HDFSRandomAccessFile*>(file->plugin_file);
  {
...
typedef struct HDFSWritableFile {
  std::string hdfs_path;
  hdfsFS fs;
  LibHDFS* libhdfs;
  hdfsFile handle;
  HDFSWritableFile(std::string hdfs_path, hdfsFS fs, LibHDFS* libhdfs,
                   hdfsFile handle)
      : hdfs_path(std::move(hdfs_path)),
        fs(fs),
        libhdfs(libhdfs),
...
void Cleanup(TF_WritableFile* file) {
  auto hdfs_file = static_cast<HDFSWritableFile*>(file->plugin_file);
  hdfs_file->libhdfs->hdfsCloseFile(hdfs_file->fs, hdfs_file->handle);
...
typedef struct HadoopFileSystemImplementation {
  LibHDFS* libhdfs;
  absl::Mutex connection_cache_lock;
  std::map<std::string, hdfsFS> connection_cache
      ABSL_GUARDED_BY(connection_cache_lock);
  HadoopFileSystemImplementation(TF_Status* status);
  ~HadoopFileSystemImplementation() {
    if (libhdfs != nullptr) {
      delete libhdfs;
    }
  }
...
HadoopFileSystemImplementation::HadoopFileSystemImplementation(
    TF_Status* status)
...
typedef struct HadoopFileSystem {
  ...
  HadoopFileSystem() : ptr(nullptr), initialized(false) {}
  HadoopFileSystemImplementation* Load(TF_Status* status) {
    absl::MutexLock l(&mu);
    if (!initialized) {
      initialized = true;
      ptr.reset(new HadoopFileSystemImplementation(status));
      if (TF_GetCode(status) != TF_OK) {
        ptr.reset(nullptr);
        return nullptr;
      }
    ...
    if (ptr.get() == nullptr) {
      TF_SetStatus(status, TF_INTERNAL,
                   "HadoopFileSystem is not initialized correctly");
      return nullptr;
    }
    TF_SetStatus(status, TF_OK, "");
    return ptr.get();
...
hdfsFS Connect(tf_hdfs_filesystem::HadoopFileSystemImplementation* hadoop_file,
...
void Init(TF_Filesystem* filesystem, TF_Status* status) {
  filesystem->plugin_filesystem = new HadoopFileSystem();
  TF_SetStatus(status, TF_OK, "");
}

void Cleanup(TF_Filesystem* filesystem) {
  auto hadoop_file =
      static_cast<HadoopFileSystem*>(filesystem->plugin_filesystem);
  delete hadoop_file;
}
...
void NewRandomAccessFile(const TF_Filesystem* filesystem, const char* path,
  ...
  file->plugin_file = new tf_random_access_file::HDFSRandomAccessFile(
      path, hdfs_path, fs, libhdfs, handle);
  TF_SetStatus(status, TF_OK, "");
}
...
void ProvideFilesystemSupportFor(TF_FilesystemPluginOps* ops, const char* uri) {
  ...
  ops->filesystem_ops->path_exists = tf_hdfs_filesystem::PathExists;
  ops->filesystem_ops->get_file_size = tf_hdfs_filesystem::GetFileSize;
  ops->filesystem_ops->stat = tf_hdfs_filesystem::Stat;
  ops->filesystem_ops->get_children = tf_hdfs_filesystem::GetChildren;
  ops->filesystem_ops->translate_name = tf_hdfs_filesystem::TranslateName;
}
